  ...
  return (
    <div>
      ...
      <main className="w-full bg-[#121212] text-neutral-300 min-h-screen p-8 md:p-20 font-sans selection:bg-red-900 selection:text-white">
        {project.banner && (
          <div
            className={`absolute inset-0 z-0 pointer-events-none transition-opacity duration-1000 ease-in-out ${bannerLoaded ? "opacity-[0.5]" : "opacity-0"}`}
          >
            <Image
              src={project.banner}
            ...
            />
            <div className="absolute inset-0 bg-linear-to-b from-transparent via-[#121212]/50 to-[#121212]" />
          </div>
        )}
        ...
        <header className="z-10 order-b border-red-950 pb-10 mb-16 flex flex-col md:flex-row justify-between items-start gap-8">
          <div>
            <p className="text-red-700 font-mono text-xs mb-4 tracking-widest uppercase">
              File_{project.id} || Archived
            </p>
        ...
        <div className="z-10 grid grid-cols-1 md:grid-cols-12 gap-12 transition-all duration-500">
          ...
          <div
            className={`${isFullScreen ? "md:col-span-12" : "md:col-span-8"} flex flex-col transition-all duration-500`}
          >
            <div className="z-10 flex items-center gap-6 mb-8 border-b border-neutral-800/50 pb-4">
              <div className="flex items-center gap-6">
                <button
                  ...
                  className={`flex items-center gap-2 text-xs uppercase tracking-widest transition-all duration-300 ${
                    activeView === "visual"
                      ? "text-white border-b-2 border-red-800 pb-4.25 -mb-4.5"
              ...
              >
                {isFullScreen ? (
                  ...
                  <>
                    <Maximize size={14} />{" "}
                    <span className="hidden sm:inline">Increase Focus</span>
                  </>
                )}
              </button>
            </div>
            ...
            {activeView === "visual" && (
              <div
                className={`columns-2 lg:columns-4 gap-3 md:gap-4 animate-in fade-in duration-500 ${isFullScreen ? "lg:columns-6" : ""}`}
              >
                    ...
                    {item.type === "video" ? (
                      <div className="w-full bg-neutral-900 border border-neutral-800 rounded-sm relative overflow-hidden group">
                        <div className="aspect-video flex items-center justify-center cursor-pointer">
                          <PlayCircle
                            size={48}
                            className="text-red-600 opacity-50 group-hover:opacity-100 transition-opacity"
                          />
                        </div>
                      </div>
                    ) : (
                ...
                {(!shuffledMedia || shuffledMedia.length === 0) && (
                  <div className="break-inside-avoid w-full aspect-square bg-neutral-900/30 border border-neutral-800 flex items-center justify-center">
                    <span className="text-neutral-700 text-xs font-mono tracking-tighter uppercase">
                      No_Assets_Logged
                    </span>
                  </div>
                )}
            ...
            {activeView === "doc" && (
              <div
                className={`z-20 space-y-6 animate-in fade-in duration-500 ${isFullScreen ? "max-w-4xl mx-auto" : "max-w-3xl"}`}
              >
                ...
                <article className="prose prose-invert prose-neutral text-neutral-400 leading-relaxed max-w-none indent-paragraphs">
                  <ReactMarkdown>
                    {project.documentation ||
                      "We're trying our best to provide the best experience for you. Detail Documentation compilation is ongoing."}
                  </ReactMarkdown>
                </article>
              </div>
            )}
